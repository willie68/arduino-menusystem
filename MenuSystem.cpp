...
MenuComponent::MenuComponent(const char* name, SelectFnPtr select_fn) :
		_name(name), _has_focus(false), _is_current(false), _select_fn(
				select_fn) {
}
...
Menu::Menu(const char* name, SelectFnPtr select_fn) :
		MenuComponent(name, select_fn), _p_current_component(nullptr), _menu_components(
				nullptr), _p_parent(nullptr), _num_components(0), _current_component_num(
				0), _previous_component_num(0) {
}
...
BackMenuItem::BackMenuItem(const char* name, SelectFnPtr select_fn,
		MenuSystem* ms) :
		MenuItem(name, select_fn), _menu_system(ms) {
}
...
NumericMenuItem::NumericMenuItem(const char* basename, SelectFnPtr select_fn,
		float value, float min_value, float max_value, float increment,
		FormatValueFnPtr format_value_fn) :
		MenuItem(basename, select_fn), _value(value), _min_value(min_value), _max_value(
				max_value), _increment(increment), _format_value_fn(
				format_value_fn) {
	if (_increment < 0.0)
...
void TextEditMenuItem::render(MenuComponentRenderer const& renderer) const {
	renderer.render_text_edit_menu_item(*this);
}
...
MenuSystem::MenuSystem(MenuComponentRenderer const& renderer) :
		_p_root_menu(new Menu("", nullptr)), _p_curr_menu(_p_root_menu), _renderer(
				renderer) {
}
